<?php
/* ===========================================================================
 *  Traitement du formulaire de demande d'intervention et de devis.
 *
 ...
 *  régénérée à chaque build. La source est static/envoi-demande.php.
 *
...
const DESTINATAIRE   = '{{EMAIL_DEVIS}}';
const EXPEDITEUR     = '{{EMAIL_EXPEDITEUR}}';
const NOM_SITE       = '{{NOM_COMMERCIAL}}';
const TELEPHONE      = '{{TELEPHONE}}';
const TELEPHONE_E164 = '{{TELEPHONE_E164}}';
define('PHOTO_MAX_MO', max(1, (int) '{{DEVIS_PHOTO_MAX_MO}}'));
...
{
    return sys_get_temp_dir() . '/demande-' . sha1(adresse_ip() . '|' . NOM_SITE) . '.txt';
}
...
<section>
  <div class="wrap" style="max-width:44em">
    ...
    <p>
      ...
      <a class="btn btn-ghost" href="/devis">Revenir au formulaire</a>
    </p>
...
if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
    header('Location: /devis', true, 303);
    exit;
}
...
$types_autorises = [
    'wc'                      => 'WC ou toilettes bouchés',
    'evier'                   => 'Évier bouché',
    'lavabo'                  => 'Lavabo bouché',
    'douche'                  => 'Douche bouchée',
    'baignoire'               => 'Baignoire bouchée',
    'cuisine'                 => 'Réseau de cuisine bouché',
    'canalisation'            => 'Canalisation intérieure bouchée',
    'canalisation-exterieure' => 'Canalisation ou regard extérieur bouché',
    'remontee'                => "Remontée d'eau ou d'odeurs",
    'curage'                  => 'Curage ou hydrocurage préventif',
    'camera'                  => 'Inspection caméra / recherche de bouchon',
    'pompage'                 => 'Pompage (fosse, bac, cave inondée)',
    'assainissement'          => 'Assainissement individuel ou collectif',
...
if (!isset($types_autorises[$type])) {
    $erreurs[] = 'Merci de choisir le type de problème dans la liste.';
}

/* Le secteur d'intervention est strictement limité à six départements.
   Une demande hors zone n'est pas rejetée en silence : le visiteur est
   informé, et on lui évite d'attendre une réponse qui ne viendrait pas. */
...
$departement = champ('departement', 4);
if (!isset($departements_autorises[$departement])) {
    $erreurs[] = "Nous n'intervenons que dans les Côtes-d'Armor (22), le Finistère (29), l'Ille-et-Vilaine (35), le Morbihan (56), la Loire-Atlantique (44) et le Maine-et-Loire (49). Merci de choisir votre département dans la liste.";
}

/* Cohérence entre le code postal saisi et le département choisi : une
   coquille ici enverrait un technicien à deux cents kilomètres. */
if ($code_postal !== '' && $departement !== '' && isset($departements_autorises[$departement])
    && substr($code_postal, 0, 2) !== $departement) {
    $erreurs[] = 'Le code postal ' . $code_postal . ' ne correspond pas au département sélectionné. Corrigez l\'un ou l\'autre.';
}

$urgences_autorisees = [
    'immediate' => 'Urgent — débordement en cours, intervention immédiate',
...
$libelle_dept    = $departements_autorises[$departement];
$prefixe         = ($urgence === 'immediate') ? '[URGENT] ' : '';

$sujet = $prefixe . 'Demande — ' . $libelle_type . ' — ' . $ville . ' (' . $departement . ')';

$corps = "Nouvelle demande déposée sur " . NOM_SITE . "\n"
       . str_repeat('=', 60) . "\n\n"
       . "Nom .............. : {$nom}\n"
       . "Téléphone ........ : {$telephone_saisi}\n"
       . "E-mail ........... : " . ($email !== '' ? $email : 'non renseigné') . "\n"
       . "Ville ............ : {$ville}" . ($code_postal !== '' ? " ({$code_postal})" : '') . "\n"
       . "Département ...... : {$libelle_dept}\n"
       . "Type de problème . : {$libelle_type}\n"
...
if (!$envoye) {
    error_log('[demande] Échec de mail() vers ' . DESTINATAIRE);
    page_erreur([
